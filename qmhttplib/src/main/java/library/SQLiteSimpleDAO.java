...
import library.util.SimplePreferencesUtil;
...
//        this(tClass, context, "DBUserID_" + DiscontoApplication.getUserPrefs().id().get());

//        if (DiscontoApplication.getUserPrefs().id().get().isEmpty()) {
//            throw new RuntimeException("SQLiteSimpleDAO user is NuLL");
//        }
//
//
...
//        for (Field field : ReflectionUtils.getDeclaredFieldsWithSuperclass(tClass)) {
        for (Field field : tClass.getDeclaredFields()) {

            Column fieldEntityAnnotation = field.getAnnotation(Column.class);
//            Column fieldEntityAnnotation = ReflectionUtils.getColumnAnnotation(field);
...
//                    Column annotationColumn = ReflectionUtils.getColumnAnnotation(field);
...
//        for (Field field : ReflectionUtils.getDeclaredFieldsWithSuperclass(object.getClass())) {
